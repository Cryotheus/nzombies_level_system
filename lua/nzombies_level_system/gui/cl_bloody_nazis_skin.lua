local delayed_derma_hooks = {
	NumSlider --we want to Derma_Hook once the label is made
}
...
SKIN.PrintName		= "Bloody nZombies"
SKIN.Author		= "Cryotheum"
SKIN.DermaVersion	= 1
SKIN.GwenTexture	= Material("gui/nzombies_level_system/skin.png")
...
function SKIN:PaintShadow(panel, w, h) self.tex.Shadow(0, 0, w, h) end

function SKIN:PaintFrame(panel, w, h)
	if panel.m_bPaintShadow then
		local wasEnabled = DisableClipping(true)
		
		self.tex.Shadow(-4, -4, w + 10, h + 10) --this turns off clipping?
		DisableClipping(wasEnabled)
	end

	if panel:HasHierarchicalFocus() then self.tex.Window.Normal(0, 0, w, h)
	else self.tex.Window.Inactive(0, 0, w, h) end
end
...
function SKIN:PaintCheckBox(panel, w, h)
	if panel:GetChecked() then
		if panel:GetDisabled() then self.tex.CheckboxD_Checked(0, 0, w, h)
		else self.tex.Checkbox_Checked(0, 0, w, h) end
	else
		if panel:GetDisabled() then self.tex.CheckboxD(0, 0, w, h)
		else self.tex.Checkbox(0, 0, w, h) end
	end
end

function SKIN:PaintRadioButton(panel, w, h)
	if panel:GetChecked() then
		if panel:GetDisabled() then self.tex.RadioButtonD_Checked(0, 0, w, h)
		else self.tex.RadioButton_Checked(0, 0, w, h) end
	else
		if panel:GetDisabled() then self.tex.RadioButtonD(0, 0, w, h)
		else self.tex.RadioButton(0, 0, w, h) end
	end
end

function SKIN:PaintExpandButton(panel, w, h)
	if not panel:GetExpanded() then self.tex.TreePlus(0, 0, w, h)
	else self.tex.TreeMinus(0, 0, w, h) end
end
...
function SKIN:PaintMenu(panel, w, h)
	if panel:GetDrawColumn() then self.tex.MenuBG_Column(0, 0, w, h)
	else self.tex.MenuBG(0, 0, w, h) end
end
...
function SKIN:PaintMenuRightArrow(panel, w, h) self.tex.Menu.RightArrow(0, 0, w, h) end
...
function SKIN:PaintActiveTab(panel, w, h) self.tex.TabT_Active(0, 0, w, h) end
...
function SKIN:PaintVScrollBar(panel, w, h) self.tex.Scroller.TrackV(0, 0, w, h) end
...
function SKIN:PaintButtonLeft(panel, w, h)
	if notpanel.m_bBackground then return end
...
function SKIN:PaintListBox(panel, w, h) self.tex.Input.ListBox.Background(0, 0, w, h) end
...
function SKIN:PaintTreeNodeButton(panel, w, h)
	...
	--Don't worry this isn't working out the size every render, it just gets the cached value from inside the Label
	local w = panel:GetTextSize()

	self.tex.Selection(38, 0, w + 6, h)
end

function SKIN:PaintSelection(panel, w, h) self.tex.Selection(0, 0, w, h) end
...
function SKIN:PaintNumSlider(panel, w, h)
	surface.SetDrawColor(panel.LineColor or color_white)
...
function SKIN:PaintCollapsibleCategory(panel, w, h)
	if h < 21 then
		...
		if not panel:GetExpanded() then self.tex.Input.ComboBox.Button.Down(w - 18, 2, 15, 15) end
...
function SKIN:PaintCategoryList(panel, w, h) self.tex.CategoryList.Outer(0, 0, w, h, panel:GetBackgroundColor()) end
...
function SKIN:PaintListViewLine(panel, w, h)
	if panel:IsSelected() then self.tex.Input.ListBox.EvenLineSelected(0, 0, w, h)
	elseif panel.Hovered then self.tex.Input.ListBox.Hovered(0, 0, w, h)
	elseif panel.m_bAlt then self.tex.Input.ListBox.EvenLine(0, 0, w, h) end
end
...
function SKIN:PaintTooltip(panel, w, h) self.tex.Tooltip(0, 0, w, h) end
function SKIN:PaintMenuBar(panel, w, h) self.tex.Menu_Strip(0, 0, w, h) end

derma.DefineSkin("BloodyNazis", "A skin that fits the feel of nZombies while still being sleek. Designed for my (Cryotheum's) nZombies addons.", SKIN)